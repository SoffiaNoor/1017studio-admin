    ...
    <div class="row">
        <div class="col-lg-12 mb-lg-0 mb-4 shadow-xl">
            <div class="card p-3">
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="px-3" style="color:black">Content Portfolio
                        </h3>
                        <hr class="ms-3 mt-0" style="background-color:#1c1c1c;height:10px;border-radius:40px;width:50%">
                    </div>
                    <div class="col-sm-6"> <a class="btn btn-warning text-white" href="{{route('portfolio.create')}}"
                ...
                <div class="table-responsive">
                                ...
                                <td class="text-uppercase text-default text-xs font-weight-bolder">
                                    <div class="d-flex align-items-center">
                                        <span class="ms-3 text-xs">
                                            {{ ($portofolio->currentPage() - 1) * $portofolio->perPage() +
                                            $loop->iteration }}
                                        </span>
                                    </div>
                                </td>
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-end pt-4">
                            @if ($portofolio->currentPage() > 1)
                            <li class="page-item">
                                <a class="page-link" href="{{ $portofolio->previousPageUrl() }}" tabindex="-1">
                            ...
                            @for ($i = 1; $i <= $portofolio->lastPage(); $i++)
                                <li class="page-item {{ $i == $portofolio->currentPage() ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $portofolio->url($i) }}"
                                        style="{{ $i == $portofolio->currentPage() ? 'color:white;background-color:#1c1c1c;border:none' : '' }}">
                                        {{ $i }}
                                    </a>
                                </li>
                                @endfor

                                @if ($portofolio->currentPage() < $portofolio->lastPage())
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $portofolio->nextPageUrl() }}">
